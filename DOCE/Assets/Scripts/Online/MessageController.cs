...
/// </remarks>
public class MessageController : MonoBehaviour, IChatClientListener
{
    ...
    public void OnClickSend()
    {
        ...
        string message = UserIdFormPanel.GetComponent<InputField>().text;
    ...
    public void OnSubscribed(string[] channels, bool[] results)
    {
        ...
        */
        ...
        bool found = this.chatClient.TryGetChannel(channels[0], out channel);
        if (!found)
        {
            Debug.Log("ShowChannel failed to find channel: " + channels[0]);
            return;
        }
        ...
        set = publicChannel.Subscribers;
        Debug.Log(publicChannel.Name);
        Debug.Log("Public count" + publicChannel.Subscribers.Count);

        Debug.Log(publicChannel.PublishSubscribers.ToString());

        foreach(string pName in publicChannel.Subscribers)
        {
            Debug.Log(pName.ToString());
        }
        Debug.Log("Checked public");
    ...
    public void OnPrivateMessage(string sender, object message, string channelName)
    {
        ...
        Debug.Log("Private message from " + sender  + " to " + UserName + " > "  + message.ToString() + "channel: " + channelName);

        //ACTIVATE MESSAGE;
        if(sender.ToString() != UserName) {
            PopUpMessage(message.ToString(), sender.ToString());
        }
    ...
    {
        Debug.Log("Show Channel!: " + channelName);
    ...
    [SerializeField]
    private OnlineLobbyController lobbyController;
    ...
    public void SendInvitationForRoom(string roomName)
    {

        //TO SEND MESSAGE OR INVITE
        string receiver = UserIdFormPanel.GetComponent<InputField>().text;
        ...
        this.chatClient.SendPrivateMessage(receiver, roomName, true);
        ...
        if (this.UserIdText != null)
        {
            ...
            Debug.Log("Send message to: " + UserIdText.text);
            this.chatClient.SendPrivateMessage(UserIdText.text, roomName, true);
        }
    ...
    public void OnClickAcceptInvitation(string roomName)
    {
        ...
        OnlineLobbyController lobbyC = FindObjectOfType<OnlineLobbyController>();
        lobbyC.JoinInvitation(roomName);
        popupPanel.gameObject.SetActive(false);
    }
