...
namespace Photon.Pun.UtilityScripts
{
	...
	public class PunTurnManager : MonoBehaviourPunCallbacks, IOnEventCallback
    {
        ...
        {
            Debug.Log("check :" + move.ToString() + " " + finished);
            if (IsFinishedByMe)
            {
			...
			case EvFinalMove:
				{
					...
					if (turn == this.Turn)
					{
						...
						this.TurnManagerListener.OnPlayerFinished(sender, turn, move);
                            Debug.Log(Turn + " sender >:" + sender.NickName);

					}
                       // Debug.Log("EvMoveeeeeFINALLLL!2222");

                        if (IsCompletedByAll)
					{
                            Debug.Log("Was completed by all");
        ...
        {
           // Debug.LogWarning("Event>> sender:" + photonEvent.Sender + photonEvent.ToStringFull());
    ...
    public static class TurnExtensions
    {
        ...
        public static void SetFinishedTurn(this Player player, int turn)
        {
           ...
           // Debug.Log(player.NickName + " Got Set Finish Turn PUN" + turn);
